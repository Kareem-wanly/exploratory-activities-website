...
function showActivity($activityschedule_gender, $activityschedule_Health_status, $activityschedule_physical_state, $activity_status)
{
    require 'connect.php';
    try {
        $sql = "SELECT activityschedule.*, users.userFullname FROM activityschedule INNER JOIN
...
function searchActivities($severity_level = 0, $activityschedule_Health_status = 0, $activityschedule_physical_state = 0, $activity_status)
{
    require 'connect.php';
    try {
        $sql = "SELECT activityschedule.*, users.userFullname FROM activityschedule INNER JOIN
                users ON activityschedule.userID = users.userID WHERE activity_status= :activity_status";
...
function viewActivity($userID = null, $ActivityID = null)
{
    ...
    if ($userID == null && $ActivityID == null) {
        ...
        try {
            $sql = 'SELECT activityschedule.*, users.UserFullname FROM activityschedule INNER JOIN users 
            ON activityschedule.userID = users.userID WHERE activityschedule.userID=:userID';
            $stmt = $conn->prepare($sql);
            $stmt->execute([
                ':userID' => $userID
            ]);
...
function search_supervisor($searchField)
{
    require 'connect.php';
    try {
        $sql = "SELECT * FROM users WHERE userName LIKE '%$searchField%' AND userType=2";
        $stmt = $conn->prepare($sql);
        $stmt->execute();
        return $stmt;
    } catch (PDOException $e) {
